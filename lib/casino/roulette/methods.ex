defmodule Casino.Roulette.Methods do
  ...
  def zero(bet) do
    {:ok, %Bet{id: 0, name: "Zero", numbers: [0], bet: bet, payout: 35}}
  end

  @spec straight_up(integer, integer) :: {:ok, Bet.t}
  def straight_up(number, bet) do
    {:ok, %Bet{id: 1, name: "Straight Up", numbers: [number], bet: bet, payout: 35}}
  end
  ...
  def top_line(bet) do
    {:ok, %Bet{id: 6, name: "Top Line", numbers: [0, 1, 2, 3], bet: bet, payout: 6}}
  end
  ...
  def odd_numbers(bet) do
    {:ok, %Bet{id: 14, name: "Odd Numbers", numbers: (for n <- 1..18, do: (n * 2) - 1), bet: bet, payout: 1}}
  end

  @spec even_numbers(integer) :: {:ok, Bet.t}
  def even_numbers(bet) do
    {:ok, %Bet{id: 15, name: "Even Numbers", numbers: (for n <- 1..18, do: (n * 2)), bet: bet, payout: 1}}
  end

  @spec red_numbers(integer) :: {:ok, Bet.t}
  def red_numbers(bet) do
    {:ok, %Bet{id: 16, name: "Red Numbers", numbers: reds, bet: bet, payout: 1}}
  end

  @spec black_numbers(integer) :: {:ok, Bet.t}
  def black_numbers(bet) do
    {:ok, %Bet{id: 17, name: "Black Numbers", numbers: blacks, bet: bet, payout: 1}}
  end

  @spec low_numbers(integer) :: {:ok, Bet.t}
  def low_numbers(bet) do
    {:ok, %Bet{id: 18, name: "Low Numbers", numbers: (for n <- 1..18, do: n), bet: bet, payout: 1}}
  end

  @spec high_numbers(integer) :: {:ok, Bet.t}
  def high_numbers(bet) do
    {:ok, %Bet{id: 19, name: "Low Numbers", numbers: (for n <- 19..36, do: n), bet: bet, payout: 1}}
  end
end
